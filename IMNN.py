...
class IMNN():
    def __init__(n, parameters):
        ...
        n.sess, n.x, n.x_central, n.central_indices, n.derivative_indices, n.θ_fid, n.prior, n.x_m, n.x_p, n.dd, n.dropout, n.output, n.F, n.μ, n.C, n.iC, n.dμdθ, n.Λ, n.test_F, n.test_μ, n.test_C, n.test_iC, n.test_dμdθ, n.test_Λ, n.MLE, n.AL, n.x_check, n.C_check, n.Q, n.backpropagate, n.saver, n.history = u.initialise_variables()
    ...
    def calculate_asymptotic_likelihood(n, output):
        ...
        asymptotic_likelihood = tf.exp(- 0.5 * (tf.expand_dims(tf.expand_dims(tf.einsum("ij,ij->i", output, tf.einsum("ij,kj->ki", n.test_iC, output)) + tf.einsum("i,ji->j", n.test_μ, tf.einsum("ij,kj->ki", n.test_iC, output)) + tf.einsum("ij,j->i", output, tf.einsum("ij,j->i", n.test_iC, n.test_μ)) + tf.einsum("i,i->", n.test_μ, tf.einsum("ij,j->i", n.test_iC, n.test_μ)), 1), 2) - tf.einsum("ijk,ij->ijk", Δθ, tf.einsum("ij,kj->ki", n.test_dμdθ, tf.einsum("ij,kj->ki", n.test_iC, output))) - tf.einsum("ijk,ij->ijk", Δθ, tf.einsum("ij,kj->ik", output, tf.einsum("ij,kj->ki", n.test_iC, n.test_dμdθ))) + tf.einsum("ijk,j->ijk", Δθ, tf.einsum("ij,j->i", n.test_dμdθ, tf.einsum("ij,j->i", n.test_iC, n.test_μ))) + tf.einsum("ijk,j->ijk", Δθ, tf.einsum("i,ji->j", n.test_μ, tf.einsum("ij,kj->ki", n.test_iC, n.test_dμdθ))) + tf.einsum("ijk,ijk->ijk", Δθ, tf.einsum("ijk,j->ijk", Δθ, tf.einsum("ij,ij->i", n.test_dμdθ, tf.einsum("ij,kj->ki", n.test_iC, n.test_dμdθ))))))
    ...
    def maximum_likelihood_estimate(n, output):
        ...
        return tf.expand_dims(n.θ_fid, 0) + tf.einsum("ij,kj->ki", iF, tf.einsum("ij,kj->ki", tf.einsum("ij,kj->ki", n.test_iC, n.test_dμdθ), output - tf.expand_dims(n.test_μ, 0)))
    ...
    def loss(n, F):
        ...
        Λ = tf.multiply(tf.constant(-0.5, dtype = n._FLOATX), tf.square(IFI))# + tf.exp(-10.*tf.matrix_determinant(n.C))
        if n.covariance_check:
            Λ = Λ - tf.square(tf.matrix_determinant(n.Q) - tf.matrix_determinant(n.C_check))#tf.nn.relu(tf.matrix_determinant(n.Q) - tf.matrix_determinant(n.C_check))
        return Λ
        ...
        for epoch in tq:
            ...
            if n.x_central.op.type != 'Placeholder':
                if n.covariance_check and n.x_check.op.type == "Placeholder":
                    ...
                    train_F = n.sess.run(n.F, feed_dict = {n.central_indices: central_indices[combination * n.n_s: (combination + 1) * n.n_s].reshape((n.n_s, 1)), n.derivative_indices: derivative_indices[combination * n.n_p: (combination + 1) * n.n_p].reshape((n.n_p, 1)), n.dropout: 1., n.x_check: data["x_check"]})
                else:
                    for combination in range(n_train):
            ...
            n.history["det(F)"].append(np.linalg.det(train_F))
            if history:
                if test:
                    if n.x_central.op.type != 'Placeholder':
                        if n.covariance_check and n.x_check.op.type == "Placeholder":
                            μ, C, dμdθ, Λ, C_check, test_F, test_μ, test_C, test_dμdθ, test_Λ = n.sess.run([n.μ, n.C, n.dμdθ, n.Λ, n.C_check, n.test_F, n.test_μ, n.test_C, n.test_dμdθ, n.test_Λ], feed_dict = {n.central_indices: central_indices[combination * n.n_s: (combination + 1) * n.n_s].reshape((n.n_s, 1)), n.derivative_indices: derivative_indices[combination * n.n_p: (combination + 1) * n.n_p].reshape((n.n_p, 1)), n.dropout: 1., n.x_check: data["x_check"]})
                        elif n.covariance_check and n.x_check.op.type != "Placeholder":
                        ...
                        if n.covariance_check:
                            μ, C, dμdθ, Λ, C_check, test_F, test_μ, test_C, test_dμdθ, test_Λ = n.sess.run([n.μ, n.C, n.dμdθ, n.Λ, n.C_check, n.test_F, n.test_μ, n.test_C, n.test_dμdθ, n.test_Λ], feed_dict = {n.x_central: data['x_central_test'], n.x_m: data['x_m_test'], n.x_p: data['x_p_test'], n.dropout: 1., n.x_check: data["x_check"]})
                        else:
                    ...
                    if n.covariance_check:
                        n.history["C check"].append(C_check)
                        n.history["det(C check)"].append(np.linalg.det(C_check))
