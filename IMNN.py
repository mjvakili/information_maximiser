...
class IMNN():
    ...
    def inverse_covariance(n, a):
        ...
        iD = tf.identity(tf.matrix_diag(1. / tf.sqrt(tf.diag_part(C))), name = 'correlation_rescaling')
        Cor = tf.einsum("ij,kj->ik", tf.einsum("ij,kj->ik", iD, C), iD, name = 'central_correlation')
        Cor = tf.matrix_band_part(Cor, 0, -1)
        Cor = 0.5 * (Cor + tf.transpose(Cor))
        iCor = tf.matrix_inverse(Cor)
        iCor = tf.matrix_band_part(iCor, 0, -1)
        iCor = tf.identity(0.5 * (iCor + tf.transpose(iCor)), name = 'inverse_central_covariance')
        iC = tf.einsum("ij,kj->ik", tf.einsum("ij,kj->ik", iD, iCor), iD, name = 'central_correlation')
